        ...
        <div>
          {precio.descuentoPct > 0 && (
            <span className="mini suave" style={{ textDecoration: 'line-through', marginRight: 8 }}>
              {formatoMoneda(precio.precioBaseMxn, 'MXN', 0)}
            </span>
          )}
          <span className="cifra" style={{ fontSize: 18, fontWeight: 600 }}>
            {formatoMoneda(precio.precioFinalMxn, 'MXN', 0)}
          </span>
        </div>
        {precio.descuentoPct > 0 && (
          <div className="mini suave">{t('consultoria.tuDescuento', { pct: precio.descuentoPct })}</div>
        )}
      </div>
